...
final Map<Uri, Set<({String key, RegExp pathRegexp})>> _groupedUris = {
  _homeUri: {
...
class Aggregator {
  ...
  (Future<void>, Set<Uri>)? _pending;

  final Map<Set<Uri>, ({Uri groupUri, GroupedFuture future, int nbExtracted})> _groupRequests = {};

  Future<T> call<T>(Uri uri, {required T Function(Map<String, dynamic>) mapper}) async {
    _pending ??= (Future<void>.delayed(const Duration(milliseconds: 50)), <Uri>{});
    _pending!.$2.add(uri);
