// Below is the original cupertino nav bar code from the Flutter repository.
// https://github.com/flutter/flutter/blob/beta/packages/flutter/lib/src/cupertino/nav_bar.dart
...
Widget _wrapWithBackground({
  ...
  // LICHESS
  // don't blur the background if it's already opaque or during hero transitions.
  if (backgroundColor.alpha == 0xFF || !updateSystemUiOverlay) {
...
class CupertinoNavigationBar extends StatefulWidget
  ...
  const CupertinoNavigationBar({
    ...
    this.transparentUnscrolledBackground = true,
    this.brightness,
    this.padding,
  ...
  /// Controls whether the navigation bar's background is transparent in the
  /// initial (no content scrolled under) state.
  ///
  /// Defaults to true.
  final bool transparentUnscrolledBackground;
...
class _CupertinoNavigationBarState extends State<CupertinoNavigationBar> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color effectiveBackgroundColor = widget
            .transparentUnscrolledBackground
        ? Color.lerp(
...
class CupertinoSliverNavigationBar extends StatefulWidget {
  ...
  const CupertinoSliverNavigationBar({
    ...
    this.heroTag = _defaultHeroTag,
    this.stretch = false,
    // LICHESS (- suppressed the assertion)
  });
...
class _CupertinoSliverNavigationBarState
  ...
  Widget build(BuildContext context) {
    final _NavigationBarStaticComponents components =
      ...
      padding: widget.padding,
      // LICHESS
      large: widget.largeTitle != null || widget.automaticallyImplyTitle,
    );
...
class _LargeTitleNavigationBarSliverDelegate
  ...
  double get maxExtent =>
      persistentHeight +
      (components.largeTitle != null ? _kNavBarLargeTitleHeightExtension : 0);
  ...
  Widget build(
    ...
    // LICHESS
    final double scrollTransparentThreshold =
        (maxExtent - minExtent - _kNavBarShowLargeTitleThreshold).abs();
    final bool showLargeTitle = shrinkOffset < scrollTransparentThreshold;
    final double transparentAnimationValue = shrinkOffset >
    ...
    final Color expandedBackgroundColor =
        expandedTransparent ? backgroundColor.withOpacity(0) : backgroundColor;

    final Widget navBar = _wrapWithBackground(
      border: expandedTransparent
          ? Border.lerp(border?.scale(0), border, transparentAnimationValue)
          : border,
