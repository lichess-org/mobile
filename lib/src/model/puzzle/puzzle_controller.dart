...
class PuzzleController extends Notifier<PuzzleState> {
  ...
  /// Loads [context]'s puzzle directly into view mode with its solution merged in, for reviewing
  /// an already-solved puzzle. Unlike [onLoadPuzzle] it never starts play nor emits a
  /// [PuzzleResult].
  void loadForReview(PuzzleContext context) {
    ...
    // _mergeSolution reads the freshly assigned state, so this cannot be folded into a single
    // state write.
    _mergeSolution();
