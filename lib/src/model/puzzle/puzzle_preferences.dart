...
/// Maximum number of puzzles a single batch request can return, whatever `nb` asks for
/// (`nb.atMost(50)` in lila's `Puzzle` controller).
const kServerPuzzleBatchCap = 50;

/// Offline-queue length of the angles the configurable size does not apply to.
///
/// Deliberately equal to [kServerPuzzleBatchCap]: a single request fills such a queue exactly, so
/// it never keeps a standing deficit.
const kFixedOfflineQueueLength = kServerPuzzleBatchCap;

/// Whether the configurable offline-queue size ([PuzzlePrefs.nbOfflinePuzzles]) applies to [angle].
/// Where it doesn't, the queue stays at [kFixedOfflineQueueLength].
bool isConfigurableOfflineQueueAngle(PuzzleAngle angle) =>
    angle == const PuzzleTheme(PuzzleThemeKey.mix);

/// The offline-queue length for [angle]: the configurable [nbOfflinePuzzles] where
/// [isConfigurableOfflineQueueAngle] allows it, otherwise the fixed [kFixedOfflineQueueLength].
