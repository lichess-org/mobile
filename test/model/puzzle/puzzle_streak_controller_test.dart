...
void main() {
  ...
  group('PuzzleStreakController local-first fetching', () {
    ...
    test('an anonymous session prefetches with a single /api/puzzle/many request', () async {
      // `/api/puzzle/many` is open to anonymous sessions (they pay a per-IP rate limit cost that
      // the signed-in app does not), so offline look-ahead works signed out too.
      final ids = ['AAAAA', 'BBBBB', 'CCCCC', 'DDDDD', 'EEEEE'];
      final requests = _RequestLog();
    ...
    test('a look-ahead refill the server rejects is not retried on every advance', () async {
      // A failed refill deliberately leaves the warmed range where it was, which keeps the margin
      // check in _advance true for the rest of the run. Without a back-off that means one full
      // window request per solved puzzle — the shape a session hits whenever the endpoint keeps
      // refusing it, a 429 against the shared per-IP budget being the likeliest cause.
      ...
      // Let any refill fired by the last advance land.
      await Future<void>.delayed(const Duration(milliseconds: 50));

      expect(
        manyRequests,
        1,
        reason: 'the rejected refill must back off, not fire once per solved puzzle',
      );
    });

    test('ids dropped from a /api/puzzle/many response are not counted as warmed', () async {
      // The endpoint silently drops any id it cannot serve — an unknown one, but also a valid one
      // whose game it failed to read — so a 200 can come back with holes. Treating the response as
      // a fully warmed window would promise an offline run that stalls dead in the middle of it,
      // and the hole would never be re-requested because windows only move forward.
      ...
      final puzzleStorage = await container.read(puzzleStorageProvider.future);
      // The rest of the window did land, but the dropped id is genuinely not cached.
      expect(await _waitForCached(puzzleStorage, PuzzleId(ids[31])), isNotNull);
      expect(await puzzleStorage.fetch(puzzleId: PuzzleId(dropped)), isNull);

      final notifier = container.read(puzzleStreakControllerProvider.notifier);

      // While the streak is still short of the hole, a refill would only re-ask for the id the
      // server just declined, so no further request is made.
      for (var i = 0; i < 8; i++) {
        expect(await notifier.next(), StreakAdvance.advanced);
      }
      await Future<void>.delayed(const Duration(milliseconds: 50));
      ...
      // Past the hole, the buffer beyond it must still be warmed — which only happens if the
      // warmed range stopped at the hole instead of covering the whole short response.
      expect(await notifier.next(), StreakAdvance.advanced);
      for (var i = 0; i < 100 && requests.manyIds.length < 2; i++) {
        await Future<void>.delayed(const Duration(milliseconds: 10));
      }
      expect(requests.manyIds, hasLength(2));
      expect(requests.manyIds.last, isNot(contains(dropped)));

      // Online, the streak still walks onto the dropped puzzle via the single-puzzle fallback.
      expect(await notifier.next(), StreakAdvance.advanced);
    ...
    test('overlapping look-ahead refills issue a single /api/puzzle/many request', () async {
      // The refill fired by build() and the one fired by the next advance overlap: both would
      // see the same empty cache and request the same ids, doubling an anonymous session's rate
      // limit draw for nothing.
      ...
      final client = MockClient((request) async {
        ...
        if (path == '/api/puzzle/many') {
          final requested = request.url.queryParameters['ids']!.split(',');
          requests.manyIds.add(requested);
          // Hold the first refill open so the second is asked for while it is still in flight.
          await gate.future;
          return http.Response(_manyJson(requested), 200);
        }
        if (path.startsWith('/api/puzzle/')) {
      ...
      // Advancing wants to top the buffer up, but the first refill has not landed yet.
      expect(
    ...
    test(
      ...
      () async {
        // Signing out rebuilds the controller on the *same* notifier, so the refill still in flight
        // for the signed-in streak must neither block the anonymous streak's own refill nor publish
        // its window as the anonymous streak's warmed range — that would leave the new streak with
        // nothing cached and unplayable offline.
        ...
        final client = MockClient((request) async {
          ...
          if (path == '/api/puzzle/many') {
            final requested = request.url.queryParameters['ids']!.split(',');
            requests.manyIds.add(requested);
            // Hold every refill open, so the signed-in streak's is still in flight when the
            // anonymous streak asks for its own.
            await gate.future;
            return http.Response(_manyJson(requested), 200);
          }
          if (path.startsWith('/api/puzzle/')) {
    ...
    test('two concurrent advances move the streak only once', () async {
      // Connectivity can flap while a stuck advance is awaiting its fetch, so the reconnect
      // listener in build() can ask for a second advance for the same solved puzzle. Both landing
      // would count a puzzle that was never played.
      var online = false;
      final client = MockClient((request) async {
      ...
      // The successor is uncached, so the advance has to await a fetch — the window in which a
      // second advance can interleave.
      ...
      expect(results, [StreakAdvance.advanced, StreakAdvance.aborted]);
