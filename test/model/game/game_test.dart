...
''');
    ...
    group('moveTimes', () {
      ExportedGame gameWith({
        required String moves,
        List<int>? clocks,
        int? increment,
        int initial = 120,
      }) {
        final json = jsonDecode(_archivedGameJsonNoEvals) as Map<String, dynamic>;
        json['moves'] = moves;
        if (clocks != null) {
          json['clocks'] = clocks;
        } else {
        ...
        if (increment != null) {
          json['clock'] = {
            'initial': initial,
            'increment': increment,
            'totalTime': initial + increment * 40,
          };
        } else {
          json.remove('clock');
        }
        return ExportedGame.fromServerJson(json);
      }

      test('each side spends zero on its first move', () {
        final game = gameWith(
          moves: 'e4 e5 Nf3 Nc6',
          clocks: [12000, 12000, 11800, 11700],
          increment: 0,
        );

        expect(game.moveTimes[0], Duration.zero);
        expect(game.moveTimes[1], Duration.zero);
      });

      test('adds the increment to the time spent', () {
        final withIncrement = gameWith(
          moves: 'e4 e5 Nf3 Nc6 Bc4',
          clocks: [12000, 12000, 11800, 11700, 11500],
          increment: 1,
        );
        final withoutIncrement = gameWith(
          moves: 'e4 e5 Nf3 Nc6 Bc4',
          clocks: [12000, 12000, 11800, 11700, 11500],
          increment: 0,
        );

        expect(withIncrement.moveTimes, const [
          Duration.zero,
          Duration.zero,
          Duration(seconds: 3),
          Duration(seconds: 4),
        ...
        expect(withoutIncrement.moveTimes, const [
          Duration.zero,
          Duration.zero,
          Duration(seconds: 2),
          Duration(seconds: 3),
      ...
      test('a game that ended on time counts the whole remaining clock', () {
        final game = gameWith(
          moves: 'e4 e5 Nf3 Nc6',
          clocks: [12000, 12000, 11800, 0],
          increment: 1,
        );

        expect(game.moveTimes.last, const Duration(seconds: 121));
      });

      test('matches the emt values of the reference game', () {
        final game = ExportedGame.fromServerJson(
          jsonDecode(_archivedGameJsonNoEvals) as Map<String, dynamic>,
        );

        // Same values as the `[%emt]` comments of the PGN test above.
        expect(game.moveTimes.take(6), const [
          Duration.zero,
          Duration.zero,
          Duration(milliseconds: 2200),
          Duration(milliseconds: 2920),
          Duration(seconds: 3),
          Duration(milliseconds: 5320),
        ]);
        expect(game.moveTimes.length, game.clocks!.length);
      });

      test('is empty for a game without clocks', () {
        final game = gameWith(moves: 'e4 e5 Nf3 Nc6');
